 ...
 * @author Admin
 */
public class MqttSubscriberByDisplay implements MqttCallback, Runnable {
    ...
    public MqttSubscriberByDisplay(BlockingQueue sharedQueueDataBp, BlockingQueue sharedQueueDataSpo2,
            BlockingQueue sharedQueueDataTemp, String displayId) {
    ...
    private void subscribers() throws MqttException {
        if (StringUtil.isNullOrEmpty(this.displayId)) {
            return;
        }
        ...
        try {
            mqttClient = new MqttClient(MQTT_BROKER, Constant.DISPLAY_REQ_GATE_SENSOR /*+ "_" + this.displayId*/, new MemoryPersistence());
            mqttClient.setCallback(this);
            mqttClient.connect(conOpt);
            mqttClient.subscribe(Constant.DISPLAY_REQ_GATE_SENSOR /*+ "_" + this.displayId*/, Constant.MQTT_QOS);
            LOGGER.info("Subscribe topic [" + Constant.DISPLAY_REQ_GATE_SENSOR /*+ "_" + this.displayId*/ + "] SUCCESS!");
        } catch (Exception ex) {
            LOGGER.info("Subscribe topic [" + Constant.DISPLAY_REQ_GATE_SENSOR /*+ "_" + this.displayId*/ + "] FAILED!, ex: " + ex.toString());
        }
    }
    ...
    public static void main(String[] args) {
        LOGGER.info("test MqttSubscriberByDisplay");
